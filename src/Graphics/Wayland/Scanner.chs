...
--     registryBind :: Registry -> Word32 -> Ptr CChar -> Word32 -> IO SomeType
...
-- | Generates a type for the haskell representation of a Wayland interface.
waylandInterfaceType :: ProtocolSpec -> Interface -> Q Type
waylandInterfaceType ps iface =
  conT $ mkName $ interfaceTypeName (protocolName ps) (interfaceName iface)
...
generateTypeSig interfaceType =
  [t| $(conT $ mkName "Registry") -> -- Registry: the WL registry
      ...
      IO $(interfaceType) -- Result: a ptr to the  bound object
  |]
  ...
  concat <$> traverse registryBindInterface (getGlobalInterfaces ps)
    where
      ...
      registryBindInterface iface = do
        let iname = interfaceName iface
            exposedName = registryBindName pname iname
        ...
        foreignImport <- lift $ foreignC
          ...
          internalCName
          (generateTypeSig (waylandInterfaceType ps iface))

        exposureDec <- lift $
          (generateWrapper exposedName
                           internalCName
                           (interfaceCInterfaceName iname))
    ...
    eachGenerateEnums iface =
      ...
      fmap concat $ mapM generateEnum (interfaceEnums iface) where
        ...
        generateEnum wlenum = do
          let qname = enumTypeName (protocolName ps)
                                   (interfaceName iface)
                                   (enumName wlenum)
...
-- | We will need a pointer to the wl_interface structs, for passing to
--   wl_proxy_marshal_constructor and wl_resource_create.
--   Now, a pretty solution would construct its own wl_interface struct here.
--   But that's way too much work for me. We just bind to the one generated by
--   the C scanner.
--
--   Example:
--   If the interface is `wl_compositor`, this will generate a foreign import
--   binding for `&wl_compositor_interface`.
--
--   foreign import ccall unsafe "&wl_compositor_interface"
--     wl_compositor_c_interface :: CInterface
generateCInterfaceDecs :: ProtocolSpec -> Q [Dec]
generateCInterfaceDecs ps = mapM bindCInterface (protocolInterfaces ps) where
  bindCInterface :: Interface -> Q Dec
  bindCInterface iface =
    foreignC
      ("&"++ interfaceName iface ++ "_interface")
      (interfaceCInterfaceName (interfaceName iface))
      [t| CInterface|] -- pointer is fixed


-- Bind to an individual message
generateMethod :: ProtocolSpec -> ServerClient -> Interface -> Name -> Int
                -> Message -> ProcessWithExports [Dec]
-- index in the list used for wl_proxy_marshal arguments
generateMethod ps Server iface destroyName idx msg =
  -- every message is just some call to wl_resource_post_event
  let pname = protocolName ps
      iname = interfaceName iface
      mname = messageName msg
      hname = eventHaskName pname iname mname
      internalCName =
...
-- index in the used for wl_proxy_marshal arguments
generateMethod ps Client iface destroyName idx msg =
  ...
  let pname = protocolName ps
      ...
      hname = requestHaskName pname iname mname
      internalCName =
...
generateMethods ps sc =
  ...
  generateInterface iface = do
    ...
    theMessages <- liftM concat $ sequence
                   $ zipWith (generateMethod  ps sc iface destroyName) [0..] $
        case sc of
